...
def get_invoice_number(words):
    ...
    for i, word in enumerate(words):
        if 'invoice' in word.lower() or 'invoices' in word.lower():
            parts = word.split()
            for part in parts:
                splitted_parts = part.lower().split('inv.')
...
def get_variable_symbol(words):
    var_symbol = ''
    for i, word in enumerate(words):
        if any([kw in word.lower() for kw in ('variable', 'var.symbol', 'vs:', 'v.s.')]):
            parts = word.split()
            for part in parts:
                if part.isdigit():
                    var_symbol = part
...
def get_date_of_issue(words):
    ...
    for i, word in enumerate(words):
        if any([kw in word.lower().rstrip(':') for kw in ('date of issue', 'date of creation', 'date of creation', 'created', 'issue', 'invoicing')]):
            word = re.sub(
...
def get_due_date(words):
    ...
    date_pattern = re.compile(r"\d{1,2}\s*[-.:]\s*\d{1,2}\s*[-.:]\s*\d{2,4}")
    for i, word in enumerate(words):
        if any([kw in word.lower().rstrip(':') for kw in ('due date', 'maturity', 'maturity:', 'maturity date', 'maturity date')]):
            word = re.sub(
                r'(\d{1,2})\.\s+(\d{1,2})\.\s+(\d{2,4})', r'\1.\2.\3', word)
            word_parts = word.split()
            for part in word_parts:
...
def get_delivery_date(words):
    ...
    date_pattern = re.compile(r"\d{1,2}\s*[-.:]\s*\d{1,2}\s*[-.:]\s*\d{2,4}")
    for i, word in enumerate(words):
        if any([kw in word.lower().rstrip(':') for kw in ('delivery date', 'date of delivery', 'date of execution', 'delivery', 'delivery', 'date given',
                                                          'tax liability', 'date given', 'liability', 'taxable')]):
            word = re.sub(
                r'(\d{1,2})\.\s+(\d{1,2})\.\s+(\d{2,4})', r'\1.\2.\3', word)
            word_parts = word.split()
            for part in word_parts:
...
def get_payment_method(words):
    payment_method = ''
    for i, word in enumerate(words):
        if any([kw in word.lower() for kw in ('payment method', 'payment method', 'payment form', 'payment method', 'payment form', 'payment method', 'payment')]):
            for j in range(i, len(words)):
                next_word = words[j]
                if any(x in next_word.lower() for x in ['transfer', 'by transfer']):
                    return 'Bank Transfer'
                elif any(x in next_word.lower() for x in ['cash', 'in cash', 'in cash']):
                    return 'In Cash'
                elif any(x in next_word.lower() for x in ['cash on delivery', 'cash on delivery']):
                    return 'Cash on Delivery'
                elif any(x in next_word.lower() for x in ['card', 'by card']):
                    return 'By Card'
                elif any(x in next_word.lower() for x in ['paypal', 'by paypal']):
                    return 'PayPal'
                elif any(x in next_word.lower() for x in ['terminal', 'by terminal']):
                    return 'By Payment Terminal'
                elif any(x in next_word.lower() for x in ['offset', 'offset']):
                    return 'Offset Payment'
...
def get_total_price(words):
    ...
    for i, word in enumerate(words):
        if any([kw1 in word.lower() and kw2 in word.lower() for kw1, kw2 in (('total', '€'), ('together', '€'), ('total', 'eur'), ('together', 'eur'),
                                                                             ('total', 'amount'), ('invoiced', 'amount'), (
                                                                             'total', 'value'), ('amount', 'payment'),
                                                                             ('together', 'payment'), ('invoiced', 'value'), (
                                                                             'total', 'payment'), ('total', 'to'), ('to', 'payment'),
                                                                             ('to', 'pay'), ('total:', ''))]):
...
def get_bank(words):
    for i, word in enumerate(words):
        ...
        if 'general' in word_lower or 'vub' in word_lower:
            return 'General Credit Bank'
        if 'csob' in word_lower or 'czechoslovak' in word_lower:
            return 'CSOB'
        if 'tatrabanka' in word_lower or 'tatra' in word_lower:
            return 'Tatra Bank'
        if 'fio' in word_lower or 'fio bank' in word_lower:
            return 'Fio Bank'
        if 'unicredit' in word_lower or 'uni credit' in word_lower:
            return 'UniCredit Bank'
        if 'slovak savings' in word_lower or 'savings' in word_lower:
            return 'Slovak Savings Bank'
        if 'bank' in word_lower:
            next_word_lower = words[i + 1].lower()
            if 'general' in next_word_lower or 'vub' in next_word_lower:
                return 'General Credit Bank'
            if 'csob' in next_word_lower or 'czechoslovak' in next_word_lower:
                return 'CSOB'
            ...
            if 'unicredit' in word_lower or 'uni credit' in word_lower:
                return 'UniCredit Bank'
            if 'slovak savings' in next_word_lower or 'savings' in next_word_lower:
                return 'Slovak Savings Bank'
    return ''
...
def get_iban(words):
    ...
    for i, word in enumerate(words):
        if 'iban' in word.lower() or 'account'.rstrip(':') in word.lower():
